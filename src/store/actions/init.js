...
export const getLanguages = () => (dispatch) => {
  return apiDirectory.getAll("/languages").then((res) => {
    if (!(res.response && res.response.data)) dispatch(setLanguages(res));
  });
};
export const getAppLanguages = () => (dispatch) => {
  return apiDirectory.getAll("/app_languages").then((res) => {
    if (!(res.response && res.response.data)) dispatch(setAppLanguages(res));
  });
};
export const getCurrencies = () => (dispatch) => {
  return apiDirectory.getAll("/currencies").then((res) => {
    if (!(res.response && res.response.data)) dispatch(setCurrencies(res));
  });
};
export const getServices = () => (dispatch) => {
  return apiDirectory.getAll("/services").then((res) => {
    if (!(res.response && res.response.data)) dispatch(setServices(res));
  });
};
