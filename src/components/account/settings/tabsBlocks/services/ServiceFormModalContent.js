...
const ServiceFormModalContent = ({
  ...
  if (!defaultServiceState.AccountServicesDiscounts?.length) {
    defaultServiceState.AccountServicesDiscounts = [
      { repetitions: "", discount: "" },
    ];
  }
  if (!defaultServiceState.AccountServicesLanguages?.length) {
    defaultServiceState.AccountServicesLanguages = [
      {
  ...
  if (!defaultServiceState.AccountServicesUrgencies?.length) {
    defaultServiceState.AccountServicesUrgencies = [
      { urgency: urgency[0].value, timeframe: timeFrames[0].value, fee: "" },
    ];
  }
      ...
      <div className="inner__modal">
        ...
        >
          ...
          <div className="controls__create">
            <input
              type="submit"
              value={modalService?.id ? t("Update") : t("Create")}
