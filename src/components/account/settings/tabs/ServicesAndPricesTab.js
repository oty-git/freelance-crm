          ...
          <div className="col-12 col-xxl-11 offset-0 offset-xxl-1">
            {!(type && id) && (
              <p className="font-semibold">
                {t("Have you learn to do new services?")}
                <br />
                <a href="#" className="text-boston-blue">
                  {t("Contact")}
                </a>
                {t(
                  " the project manager to update your pricing & services list.",
                )}
              </p>
            )}
          </div>
...
const TableRow = ({ service, setModalService, deleteItem, actions }) => {
  ...
  return (
    <tr>
      <td className="main__stat">
        <div className="icon-title">
          <div className="icon-title__icon">
            <img src={service?.Service?.icon?.disk_name} />
          </div>
      ...
      <TdMoreItems
        data={service?.AccountServicesUrgencies}
        name={"urgency"}
        value={"fee"}
      />
      <TdMoreItems
        data={service?.AccountServicesDiscounts}
        name={"repetitions"}
        value={"discount"}
      />
      {!!actions && (
        <td>
          <div
            className={
              "row-group row-group--nowrap row-group--sm justify-space-around"
            }
          >
            <a href="" onClick={(e) => updateItem(e)}>
              <img src="/images/editgrey.svg" alt="" />
            </a>
            <a href="" onClick={(e) => deleteItem(e, service)}>
