...
const RegistrationFormFreelancer = () => {
    ...
    const signUp = (e) => {
        e.preventDefault();
        let tmpState = {...state};
        tmpState['AccountToServices'] = state['AccountToServices'].filter((item) => {
            return !isEmpty(item);
        });
        ...
        apiFreelancers.put(tmpState, '/signup',e).then(r => {
            if (r && r.success){
                if(tmpState.files && tmpState.files.length){
                    apiFreelancers.put(formData, `/updateAfterSignUp/${r.profile.id}`).then(r => {
                        console.log(r);
                        if (r && r.success){
                ...
                if( newState.hasOwnProperty(stateItem)){
                    ...
                    newState[stateItem] = newState[stateItem].map(currentItem=>{
                        ...
                        if (currentItem.price_calculation_type === null) {
                            currentItem = {...currentItem, error:t('*Select price calculation type to continue')}
                            ...
                            for (let language of currentItem.AccountServicesLanguages){
                                ...
                                if(!price){
                                    ...
                                    currentItem = {...currentItem, error:t('*Please fill price')}
                                }else if(currentItem.max_price
                                    && (parseFloat(price)>parseFloat(currentItem.max_price) || parseFloat(price)<parseFloat(currentItem.min_price))
                                ){
                                    currentItem = {...currentItem, error:t('*The price should be between') + ` ${currentItem.min_price} - ${currentItem.max_price}`}
                                }
                            }
                        }
    ...
    return <div className="content__form">
        <div className="container">
            <div className="application-page__row">
                <div className="application-page__form">
                        ...
                        <FormBlocksWrapper setParentState={setState} parentState={state} step={activeStep} errors={errors}/>
                        {
                            activeStep !== 3 ?
                                <FormButtons handleNext={handleNext} handlePrev={handlePrev} handleDiscard={handleDiscard}/>
                                :
                                <div className="application-form__item">
                                    <button className="button button--fluid button--surfie-green" type="button"
                                            onClick={e => signUp(e)}>
