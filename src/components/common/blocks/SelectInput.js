...
const SelectInput = ({options, className, name, changeHandler, arrayName, arrayIndex,value, labelImg, placeholder, search}) => {
    ...
    return <div className={"select js-select " + className + (isOpenSelect ? ' is-expanded' : '')} onClick={e => toggleSelect(!isOpenSelect, e)} ref={node}>
        <button className="select__label js-select-label" type="button" data-selected={items[selected]?.id ||items[selected]?.value || (items[selected] ?? t('Please select'))}>
            ...
            <span>{placeholder || items[selected]?.label || items[selected]?.name || getLabelFromString(items[selected] ?? t('Please select'))}</span>
        </button>
        <div className="select__dropdown js-select-dropdown" style={{display: (isOpenSelect ? 'block' : '')}}>
            {search && <div className="select__search">
                <input className="input input--border-alto input--sm js-select-search" type="text"
        ...
        <input className="select__input js-select-input" name={name} type="hidden" value={items[selected]?.id || items[selected]?.value || (items[selected] ?? '')}/>
    </div>;
}
