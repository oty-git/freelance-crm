  ...
  return (
    ...
    Object.keys(config).map((key) => {
      if (key.indexOf("_group") !== -1) {
        return (
          <div className={"form-group mt-30"} key={key}>
            {!!(
              config[key][Object.keys(config[key])[0]] &&
              config[key][Object.keys(config[key])[0]].group_name
            ) && (
            ...
            <div className="row " style={{ minHeight: "100px" }}>
              {Object.keys(config[key]).map((keyInner) => (
                <div
                  key={keyInner}
                  className={`col-${
                    config[key][keyInner].groupWidth ||
                    12 / Object.keys(config[key]).length
                  }`}
                >
...
const FormItemWithTranslations = ({
  config,
  column,
  data,
  dataName,
  ...
  return config.translate ? (
    <React.Fragment>
      {languages.map((language, index) => {
        ...
        let name =
          config.translate && language.code !== defaultLang
            ? "translations" + "[" + column + "]" + "[" + language.code + "]"
            : column;
        let value =
          (config.translate && language.code !== defaultLang) ||
          appLang !== defaultLang
            ? data.translations &&
              ...
              data.translations[column][language.code]
              ? data.translations[column][language.code]
              : ""
            : data[column];
        ...
        return (
          <div
            className={
              currentLang !== language.code ? "form-group d-none" : "form-group"
            }
            key={language.code}
          >
            <FormItem
...
const FormItem = ({
  ...
  useEffect(() => {
    if (config?.api) {
      config.api.getAll().then((response) => {
        if (response.rows) setOptions(response.rows);
  ...
  if (
    config.form === false ||
    (configState[column] && configState[column].defaultHidden)
  )
    return null;
          ...
          {options && options.length > 0 ? (
            config.multiple ? (
              <MultiSelectInput
                name={column}
                options={options}
                value={data[columnName || column] || data}
                inputName={dataName}
