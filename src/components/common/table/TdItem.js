...
const TdItem = ({ data, config, column }) => {
  ...
  switch (config?.type) {
      ...
      return (
        <td>
          <p>
            <img
              width={50}
              src={data[column]?.disk_name || data[column]}
              alt=""
            />
          </p>
        </td>
      ...
      return (
        <td>
          <p>
            {value &&
            value.content_type &&
            value.content_type.indexOf("image") !== -1 ? (
              <img
                width={50}
                src={value ? value.disk_name || value : ""}
                alt=""
              />
            ) : (
              <a href={value ? value.disk_name || value : ""}>
    ...
    case "select":
      ...
      return (
        <td>
          <p>{getLabelFromString(data[column])}</p>
        </td>
      );
    default:
      return (
          ...
          ? data.map((item) => {
              return (
                <div
                  ...
                  className={"elem__permission"}
                >
                  <p>{capitalizeFirstLetter(field ? item[field] : item)}</p>
                </div>
              );
            })
          : data}
