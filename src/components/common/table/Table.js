    ...
    if (formToJson) {
      ...
      for (let formKey in formData) {
        if (
          formConfig &&
          formConfig[formKey] &&
          formConfig[formKey].showHidden
        ) {
          let showHiddenKey = formConfig[formKey].showHidden;
          if (Array.isArray(showHiddenKey)) {
            showHiddenKey.forEach((showHiddenKeyInner) => {
              if (
                formConfig[showHiddenKeyInner] &&
                typeof formData[showHiddenKeyInner] === "undefined"
              ) {
                formData[showHiddenKeyInner] =
                  formConfig[formKey].type === "checkbox" ? 0 : null;
              }
            });
          } else {
            if (
              formConfig[showHiddenKey] &&
              typeof formData[showHiddenKey] === "undefined"
            ) {
              formData[showHiddenKey] =
                formConfig[formKey].type === "checkbox" ? 0 : null;
            }
          }
        }
        ...
        if (
          ...
          if (Array.isArray(showHiddenKey)) {
            showHiddenKey.forEach((showHiddenKeyInner) => {
              if (
                formConfig[showHiddenKeyInner] &&
                formData.get(showHiddenKeyInner) == null
              ) {
                formData.append(
                  showHiddenKeyInner,
                  formConfig[formKey].type === "checkbox" ? 0 : "",
                );
              }
            });
          } else {
            if (
              formConfig[showHiddenKey] &&
              formData.get(showHiddenKey) == null
            ) {
              formData.append(
                showHiddenKey,
                formConfig[formKey].type === "checkbox" ? 0 : "",
    ...
    <div className="catalog">
      ...
      <div className="catalog__box">
        <div className="catalog__content">
          <div
            className={
              "catalog__content-item tabs__content js-tabs-content is-active"
            }
          >
